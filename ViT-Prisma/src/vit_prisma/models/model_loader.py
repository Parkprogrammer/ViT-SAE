...
def load_config(
    ...
    **kwargs,
    ...
    # NOTE: I changed this rubbish code!!!
    if category == ModelCategory.TIMM:
        ...
        cfg_dict = {
            "n_layers": len(timm_model.blocks),
            "d_model": timm_model.embed_dim,
            # --- --- ---
            "n_heads": timm_model.blocks[0].attn.num_heads, # 정확한 경로로 수정
            # --- --- ---
            "d_mlp": int(timm_model.blocks[0].mlp.fc1.out_features),
            "image_size": timm_model.default_cfg['input_size'][-1],
            "patch_size": timm_model.patch_embed.patch_size[0],
            "model_name": model_name,
            "normalization_type": "LN",
            "n_classes": timm_model.num_classes,
        }
        

        new_config = HookedViTConfig(**cfg_dict)
        ## FUuck YUO!!
